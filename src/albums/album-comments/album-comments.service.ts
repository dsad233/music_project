...
export class AlbumCommentsService {
  ...
  async create(
    albumId: number,
    userId: number,
    createAlbumCommentDto: CreateAlbumCommentDto,
  ) {
    ...
    return {
      statusCode: 201,
      message: '성공적으로 앨범 댓글 생성이 완료되었습니다.',
    };
  }
  ...
  async findDeleteList(page: number, page_size: number) {
    ...
    const [findDeleted, total] = await this.albumCommentsRepository
      .createQueryBuilder('album_comments')
      .withDeleted()
    ...
    return {
      statusCode: 200,
      message: '성공적으로 삭제 예정된 앨범 댓글 전체 조회가 완료되었습니다.',
      total: total,
      pageRange: pageRange,
      data: findDeleted,
    };
  ...
  async update(
    ...
    id: number,
    userId: number,
    updateAlbumCommentDto: UpdateAlbumCommentDto,
  ) {
    const findAlbumOne = await this.albumRepository.findOne({
      where: { id: albumId, isOpen: true },
      select: ['id'],
    ...
    return {
      statusCode: 201,
      message: '성공적으로 앨범 댓글 수정이 완료되었습니다.',
    };
  }
  ...
  async remove(albumId: number, id: number) {
    ...
    return {
      statusCode: 201,
      message: '성공적으로 앨범 댓글 삭제가 완료되었습니다.',
    };
  }
