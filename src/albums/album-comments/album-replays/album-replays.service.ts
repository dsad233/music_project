...
export class AlbumReplaysService {
  ...
  async create(
    ...
    createAlbumReplayDto: CreateAlbumReplayDto,
    userId: number,
  ) {
    ...
    return {
      statusCode: 201,
      message: '성공적으로 앨범 대댓글 생성이 완료되었습니다.',
    };
  }
  ...
  async findDeleteList(page: number, page_size: number) {
    ...
    const [findDeleted, total] = await this.albumReplaysRepository
      .createQueryBuilder('album_replays')
      .withDeleted()
    ...
    return {
      statusCode: 200,
      message: '성공적으로 삭제 예정된 앨범 대댓글 전체 조회가 완료되었습니다.',
      total: total,
      pageRange: pageRange,
      data: findDeleted,
    };
  ...
  async update(
    ...
    albumCommentId: number,
    id: number,
    updateAlbumReplayDto: UpdateAlbumReplayDto,
    userId: number,
  ) {
    const findAlbum = await this.albumRepository.findOne({
      where: { id: albumId, isOpen: true },
    ...
    return {
      statusCode: 201,
      message: '성공적으로 앨범 대댓글 수정이 완료되었습니다.',
    };
  }
  ...
  async remove(albumId: number, albumCommentId: number, id: number) {
    ...
    return {
      statusCode: 201,
      message: '성공적으로 앨범 대댓글 삭제가 완료되었습니다.',
    };
  }
