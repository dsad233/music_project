...
describe('AuthService', () => {
  ...
  beforeEach(async () => {
    ...
    userInfosRepository = module.get<Repository<UserInfos>>(
      getRepositoryToken(UserInfos),
    );
  ...
  it('Auth Register Method', async () => {
    ...
    mockUsersRepository.findOne.mockImplementation(({ where }) => {
      ...
      if (where.nickname)
        return Promise.resolve({ nickname: 'test_new_nickname' });
  ...
  it('Auth Login Method', async () => {
    ...
    jest
      ...
      .mockResolvedValue(loginDto.password, users.password);
    ...
    expect(jwtService.sign).toHaveBeenCalledWith(
      ...
      { expiresIn: '1h', secret: refreshTokenSecretKey },
    );
    expect(result).toEqual({
