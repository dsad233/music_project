...
import { ENV_REFRESH_SECRET_KEY } from "src/const/keys";
...
export class TokenVerifyService {
    ...
    async verifyRefreshToken (refreshToken : string, userIp : string, userAgent : string, userId : number){
        ...
        if(cookieRefreshTokenSplit && cookieRefreshTokenSplit.length !== 3){
          throw new BadRequestException("올바르지 않는 토큰 형식입니다. 재로그인 해주세요.");
        }
        ...
        try {
          ...
          if(Date.now() > decode.exp * 1000 || Date.now() > getUserAgent["deadline"]){
