...
export class PostsService {
  ...
  async create(
    ...
    const { title, singerName, genre, lyrics, releaseDate, isOpen } =
      createPostDto;
    const musicTitle = await this.postsRepository.findOne({
      where: { title },
      withDeleted: true,
      select: ['title'],
    });
    const musicSingerName = await this.postsRepository.findOne({
      where: { singerName },
      withDeleted: true,
      select: ['singerName'],
    });
    const musicGenre = await this.postsRepository.findOne({
      where: { genre },
      withDeleted: true,
      select: ['genre'],
    });
    const Body =
      musicTitle !== null &&
      musicSingerName !== null &&
      musicGenre !== null &&
      musicTitle.title === title &&
      musicSingerName.singerName === singerName &&
  ...
  async findNotOpendList(
    ...
    return {
      statusCode: 200,
      message: '성공적으로 비공개 노래 전체 조회가 완료되었습니다.',
      total: total,
      pageRange: pageRange,
      data: findData,
    };
  ...
  async findDeletedList(
    ...
    const findData = this.postsRepository
    ...
    if (singerName) {
      findData.andWhere('posts.singerName Like :singerName', {
    ...
    return {
      statusCode: 200,
      message: '성공적으로 삭제 예정된 노래 전체 조회가 완료되었습니다.',
      total: total,
      pageRange: pageRange,
      data: result,
    };
  ...
  async myPostfindAll(
    ...
    return {
      statusCode: 200,
      message: '성공적으로 작성한 노래 목록들 전체 조회가 완료되었습니다.',
      total: total,
      pageRange: pageRange,
      data: mypostAll,
    };
  ...
  async findOne(id: number) {
    ...
    if (cached) {
      return {
        statusCode: 200,
        message: '성공적으로 노래 상세 조회가 완료되었습니다.',
        data: cached,
      };
    }
    ...
    const findPost = await this.postsRepository.findOne({
      ...
      relations: {
        postComments: {
          ...
          postCommentLikes: { users: { userInfos: true } },
          postReplays: {
            postReplayLikes: { users: { userInfos: true } },
            users: { userInfos: true },
          },
        },
        postLikes: { users: { userInfos: true } },
      },
      select: {
        id: true,
        title: true,
