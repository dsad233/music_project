...
describe('PostsService', () => {
  ...
  it('Posts Create Method', async () => {
    ...
    expect(mockPostsRepository.findOne).toHaveBeenCalledWith({ select : ['title'], where : { title : createPostDto.title }, withDeleted : true });
    expect(mockPostsRepository.findOne).toHaveBeenCalledWith({ select : ['singerName'], where : { singerName : createPostDto.singerName }, withDeleted : true });
    expect(mockPostsRepository.findOne).toHaveBeenCalledWith({ select : ['genre'], where : { genre : createPostDto.genre }, withDeleted : true });
  ...
  it('Posts findAll Method', async () => {
    ...
    const result = await postsService.findAll(page, pageSize, searchTitle, searchSingerName);
