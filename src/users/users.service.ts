...
import { ENV_PASSWORD_SALT } from 'src/utils/const/keys';
...
export class UsersService {
  ...
  async findNotOpendList(
    ...
    return {
      statusCode: 200,
      message: '성공적으로 비공개 유저 전체 조회를 완료하였습니다.',
      total: total,
      pageRange: pageRange,
      data: findData,
    };
  ...
  async findDeletedList(
    ...
    const findDeletedData = this.userRepository
    ...
    if (email) {
      findDeletedData.andWhere('users.email LIKE :email', {
        email: `%${email}%`,
      });
    }

    if (nickname) {
      findDeletedData.andWhere('users.nickname LIKE :nickname', {
        nickname: `%${nickname}%`,
      });
    }

    if (phoneNumber) {
      findDeletedData.andWhere('userInfos.phoneNumber LIKE :phoneNumber', {
    ...
    const [result, total] = await findDeletedData
      .skip(offset)
      .take(page_size)
      .getManyAndCount();
    ...
    return {
      statusCode: 200,
      message: '성공적으로 삭제 예정된 유저 전체 목록을 조회 완료하였습니다.',
      total: total,
      pageRange: pageRange,
      data: result,
    };
  ...
  async myPage(
    ...
    return {
      statusCode: 200,
      message: '성공적으로 마이페이지 조회를 완료하였습니다.',
      data: findUser,
    };
  }
  ...
  async remove(id: number) {
    ...
    const removeCacheData = await this.cacheManager.store.keys(
      `userAgent:${id}:*`,
    );
