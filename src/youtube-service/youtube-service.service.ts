...
export class YoutubeServiceService {
  ...
  async searchVideoOne(query: string) {
    ...
    const videos = await page.evaluate(() => {
      const videoElements = Array.from(document.querySelectorAll('#primary-inner'));
      return videoElements.map(video => {
        const title = video.querySelector('#title > h1 > yt-formatted-string').textContent.trim();
        const tagName = video.querySelector("#text > a").textContent.trim();
        const videoInfo = video.querySelector("#attributed-snippet-text > span > span").textContent.trim();
        const createDate = video.querySelector("#info > span:nth-child(3)").textContent.trim();
        const count = video.querySelector("#info > span:nth-child(1)").textContent.trim();
